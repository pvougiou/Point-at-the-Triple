...
local beam_sampling_params = {
    checkpoint = './checkpoints/full/without_surf_form_tuples/epoch_95.50.error_10.5543.without_surf_form_tuples.model.t7', -- The filepath to the saved pre-trained model.
    -- IMPORTANT: Make sure that the dataset that will be loaded matches the specification of the pre-trained model.
    -- dataset_path = '../../DBpedia/WikiProject Biography/WikiProject Biography (Valid)/with_years/without_surf_form_tuples/', -- The path to the folder with all the required dataset-related files.
    dataset_path = '../../DBpedia/Full (Valid)/with_years/without_surf_form_tuples/',
    -- dataset_path = '../../DBpedia/WikiProject Biography/WikiProject Biography (Valid)/with_years/with_pointers_without_surf_form_tuples/', -- The path to the folder with all the required dataset-related files.
    
    beam_size = 8, -- Sets the beam size that will be used during decoding.
...
local function main()
	...
	-- local surf_form_mask = torch.reshape(torch.cat(attn_mask:transpose(1,2), triple_to_surf_form_mask:transpose(1,2)), params.num_surf_forms * params.num_aligned_triples, params.batch_size):transpose(1,2)
	...
	if params.layers == 1 then decoder.s[0]:zero()
	else
...
:copy(candidates[{{}, (state.batchidx - 1) * params.batch_size + itemidx_in_batch, {}}])
		...
		for beamidx = 1, params.beam_size do
			...
			-- This is wrong when the next_word_indices points to the last token in the dictionary.
			-- winning_latter_words = math.max(next_word_indices[beamidx - is_beam_active[itemidx_in_batch]:sub(1, beamidx):eq(0):sum()] % params.target_vocab_size, 1)
			...
			-- if candidate_words[winning_sequence_index] == end_token then
			if winning_latter_words == end_token then
			    ...
			    if itemidx_in_batch == 7 then
			    	print('Completed summaries until now:')			    
			    	print(summaries[{{}, (state.batchidx - 1) * params.batch_size + itemidx_in_batch}])
			    	print('Probabilities of the completed summaries:')
			    	print(probabilities[{{}, (state.batchidx - 1) * params.batch_size + itemidx_in_batch}])
				print('Running candidates:')
			    	print(candidates[{{}, (state.batchidx - 1) * params.batch_size + itemidx_in_batch}])
				print('Running tmp_candidates:')
			    ...
			    -- Comment-in in case you wish to monitor the progress on the
			    -- candidate summaries (i.e. before they get completed.)
			    -- if itemidx_in_batch == 75 then
			    -- 	print('Current beamidx:')
			    -- 	print(beamidx)
			    -- 	print('winning_sequence_index')
			    -- 	print(winning_sequence_index)
			    -- 	print('winning_latter_words')
			    -- 	print(winning_latter_words)
			    -- 	print('batch_indices for this particular item in the batch (i.e. itemidx_in_batch):')
			    -- 	print(batch_indices[itemidx_in_batch])
				
			    -- 	print('candidate_words for this particular item in the batch (i.e. itemidx_in_batch):')
			    -- 	print(candidate_words)
			    -- 	-- The winning indices of the tokens in the aggregated dictionary that lead to the

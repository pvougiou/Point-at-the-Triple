...
-- Adopted from: https://github.com/wojzaremba/lstm/blob/master/base.lua
function enableDropout(node)
    if type(node) == "table" and node.__typename == nil then
        for i = 1, #node do
            node[i]:apply(enableDropout)
        end
        return
    end
...
-- Adopted from: https://github.com/wojzaremba/lstm/blob/master/base.lua
function disableDropout(node)
    if type(node) == "table" and node.__typename == nil then
        for i = 1, #node do
            node[i]:apply(disableDropout)
        end
        return
    end
    if string.match(node.__typename, "Dropout") then
